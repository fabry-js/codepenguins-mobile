...
const ChatMessage = (props: any) =>{
    ...
    if (uid === auth.currentUser?.uid){
        return (
            <div>
                <IonChip>
                    <IonChip><IonImg src={photoURL} style={{width: '100%', height: '100%'}} /></IonChip>
      ...
      <IonContent fullscreen>
        ...
        <IonItem className="sendmessageform">
            <form onSubmit={messageSubmit}>
                ...
                <IonButton expand="block" type="submit" ><IonIcon icon={sendOutline} /></IonButton>
            </form>
          </IonItem>
          <IonToast isOpen={lengthZeroToast} duration={3000} onDidDismiss={() => setLengthZeroToast(false)} color="warning" message="⚠ | Well, at least type something!" position="top" />
          <IonToast isOpen={errorToast} duration={3000} onDidDismiss={() => setErrorToast(false)} color="danger" message="⁉ | Uh-oh something bad occurred, please retry!" position="top" />
      </IonContent>
    </IonPage>
  );
};
